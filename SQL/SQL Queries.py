...
def gen_sample_queries(connection, num_queries = 1, random_queries = True):
    ...
    for query in selected_queries:
        ...
        if 'where' in specific_query and 'having' in specific_query and numeric_columns and categorical_columns:
            ...
            if random_queries:
                ...
                having_index = filtered_tokens.index('having') 
            ...
            if random_queries:
                ...
                if where_index < order_index:
                    where_tokens = filtered_tokens[where_index + 1:order_tokens]
                ...
                columns_used = ', '.join(col for col in filtered_tokens if col in numeric_columns + categorical_columns)

            if limit_value and where_value and str(limit_value) == str(where_value):

                where_value_index = filtered_tokens.index(str(where_value) in filtered_tokens)
                limit_value_index = filtered_tokens.index(str(limi_value) in filtered_tokens)

                if where_position == limit_position:
                    limit_value = next((int(v) for i, v in enumerate(filtered_tokens) if v.isdigit() and i != where_value_index), None)

            if where_col in categorical_columns:
                    where_value = f"'{where_value}'"
                
            if where_col and where_condition and where_value and order_col and order_type and limit_value:
    ...
    # order by    
            ...
            if random_queries:
                ...
                columns_used = ', '.join([col for col in numeric_columns+categorical_columns if col in filtered_tokens])
                order_col, order_type, limit_value = get_order_clause(cursor, table_choice, numeric_columns, random_queries, order_tokens, filtered_tokens)
            ...
            if limit_value:
                ...
                nl = f"{limit_text[order_type]} {limit_value} {order_col} ordered by {order_col}"

            else:
                query = f"Select {columns_used} from {table_choice} order by {order_col} {order_type};"
                nl = f"{columns_used} ordered by {order_col} in {order_type.lower()} order."
            sample_queries.append(f"NL Description: {nl}\n"
                                      f"\n"
                                      f"Query: {query}")
